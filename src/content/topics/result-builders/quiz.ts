...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "Runs the closure body on a background thread, dispatching each statement asynchronously to avoid blocking the main queue",
      "Generates Equatable conformance automatically by synthesizing == comparisons across all of the type's stored properties",
      "Caches expensive computations using memoization so the closure body re-evaluates only when its direct inputs actually change",
    ],
    answer: 0,
    explanation:
...
@SB func greeting() -> String {
  ...
  },
  {
    ...
    options: [
      ...
      "The core protocol that every SwiftUI view must conform to in order for the framework to render it on screen",
      "A property wrapper that stores view-local state externally in SwiftUI's persistent node graph across re-renders",
      "A compatibility bridge that wraps any UIKit UIView subclass for seamless embedding inside a SwiftUI view hierarchy",
    ],
    answer: 0,
    explanation:
...
@B func f() -> Int {
    ...
    options: [
      ...
      "for loops are never allowed in any result builder, regardless of which build methods are implemented",
      "It compiles and returns 6, because buildBlock receives each iteration's result and sums them",
      "You must use a while loop instead, since while is the only looping construct builders support",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "SwiftUI automatically erases each branch to AnyView, discarding static type information to allow any mix of view types",
      "Views are all the same underlying type, since the View protocol erases concrete types at the protocol boundary",
      "It doesn't — both branches of an if/else in a @ViewBuilder must resolve to the exact same concrete view type",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Skip buildBlock entirely, replacing it so the builder only needs buildExpression to produce its final result",
      "Run each expression concurrently on a background thread, collecting results in completion order",
      "Disable static type checking for the closure body, allowing untyped expressions the compiler would otherwise reject",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
