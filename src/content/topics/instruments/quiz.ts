...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "Automatically rewrites slow code by identifying hot functions and replacing their inner loops with compiler-generated SIMD equivalents during the profiling session",
      "Only checks for compiler warnings and static analysis issues without ever running the app or requiring an attached device",
      "Runs your unit tests faster by parallelizing them across available CPU cores and caching results between invocations",
    ],
    answer: 0,
    explanation:
...
fetchPosts()            1.0 ms       42.4 ms`,
    options: [
      ...
      "Total Weight — loadFeed() has the highest total, so it is the function whose body you should rewrite first",
      "Neither, you should look at object count instead to find the allocation causing the problem",
      "Total Weight, because it includes all callees and therefore represents the complete work for each call site",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Time Profiler — it samples CPU activity and also surfaces memory spikes on the same timeline",
      "Signposts — it records custom timing intervals that correlate directly with object lifetime events",
      "Network instrument, because unreleased objects often come from cached network responses",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      "Time Profiler shows anonymous stretches of samples; a signpost names a region of your own logic so Instruments can show it as a labeled interval alongside the raw CPU samples",
      "Signposts replace the need for Time Profiler entirely by synthesizing a complete call-tree view from the begin/end markers you place in your own code",
      "Signposts instruct the compiler to apply additional loop-vectorization and inlining passes to the marked region, so the instrumented build runs faster than a standard Release build",
      "Signposts are required for the app to compile in Release mode because the Release optimizer strips all debug symbols that Time Profiler depends on to attribute samples to source locations",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Nothing is wrong — subjective perception of smoothness correlates closely with the self-time numbers Instruments previously recorded and is a reliable enough proxy to skip re-profiling",
      "The sorted(by:) standard-library function is fully inlined by the compiler and can therefore never appear as a measurable bottleneck in any user-code profiling session",
      "Instruments automatically discards any recording made before a source-file edit, so the original self-time baseline is no longer available to compare against after the fix",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
