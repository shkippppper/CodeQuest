...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "Strings are immutable value types, so any subscript operation would need to return a copy rather than a reference",
      "`Int` isn't `Hashable`, so it cannot satisfy the `Index` associated type that `Collection` requires for subscripting",
      "It only works on `NSString`, which bridges Swift strings to Objective-C and adds the missing integer subscript via its API",
    ],
    answer: 0,
    explanation:
...
print(e, e.count)`,
  ...
  },
  {
    ...
    options: [
      ...
      "The slice is immediately deep-copied into a brand-new 1 MB buffer, wasting the exact same amount of memory as the original",
      "Substrings can't be stored in properties because the compiler rejects them outside of local variable scope",
      "There's no downside — the Substring is a view and the runtime automatically discards the parent once no full-String reference remains",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "O(1) — like array indexing, because Swift stores a hidden offset table alongside each String's UTF-8 buffer",
      "O(log n) — strings are stored as balanced trees of chunks, so bisection finds the right node quickly",
      "O(n^2) — each call to index(_:offsetBy:) scans from the start, and doing it repeatedly multiplies the cost",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
