...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "WebSockets eliminate the server entirely; the protocol establishes a direct peer-to-peer channel between two iOS devices without any backend infrastructure",
      "WebSockets are just a faster variant of HTTP GET that reuses the same stateless request/response model but with configurable shorter polling timeouts on the client side",
      "Polling is not possible with URLSession at all; you must use NSURLConnection's legacy scheduled-request API to issue background repeated HTTP requests on a timer",
    ],
    answer: 0,
    explanation:
...
try await task.send(.string("hello"))`,
    options: [
      ...
      "The message is internally queued by URLSession and flushed automatically once the connection upgrades to WebSocket protocol",
      "It works fine — unlike data tasks, WebSocket tasks start the handshake the moment they are created without needing resume()",
      "It throws a compile-time error because send(_:) requires an active URLSessionWebSocketDelegate before it can be called",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "receive() is a strict one-shot call that permanently closes the underlying WebSocket frame reader channel after it returns exactly one message to the caller",
      "The while-true loop is required to keep the TCP connection alive by continuously re-sending acknowledgment frames that prevent the server from timing out the idle socket",
      "A single receive() call always times out after delivering one message because URLSession enforces a hard one-message-per-task policy on WebSocket connections",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "It will deadlock immediately because the detached Task{} and the receive loop compete for the same actor executor",
      "It is identical in behavior to awaiting handleSlowly inline, since Swift concurrency serializes all Task closures on the main actor",
      "Task{} automatically limits concurrency to the number of CPU cores so at most N messages process simultaneously",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Switch from .string messages to .data messages so URLSession can batch multiple incoming frames together, reducing the total number of Task allocations per burst",
      "Call sendPing more frequently so the server detects the high round-trip rate and automatically throttles its outgoing frame rate to match the client's available processing bandwidth",
      "Remove the while-true receive loop entirely and instead rely on URLSessionWebSocketDelegate callbacks, which the system rate-limits to deliver messages sequentially without concurrency pressure",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
