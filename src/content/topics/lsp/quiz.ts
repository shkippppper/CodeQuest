...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "A subtype must override every method of its base type, providing a new implementation even when the inherited behavior is already correct and sufficient",
      "A subtype must have fewer methods than its base type so that callers cannot accidentally rely on capabilities that the narrower substituted type lacks",
      "A subtype must always be a struct, never a class, because value-type semantics prevent the aliasing bugs that make behavioral substitutability impossible to guarantee",
    ],
    answer: 0,
    explanation:
...
print(square.width, square.height)`,
    options: [
      ...
      "8, 4 — height is untouched as doubleWidth intends, because didSet on width only propagates changes when both dimensions were already equal before the assignment",
      "Compile error — Square cannot override var properties because stored property overrides require the subclass to use the same backing storage as the superclass",
      "4, 4 — width never actually changes because didSet intercepts the assignment and cancels it to keep the square's equal-dimension invariant intact",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Only weaken them, to be more flexible and accept a broader range of return states that callers can check at runtime if needed",
      "Remove them entirely as long as the method still compiles, because postconditions are informal contracts not enforced by the Swift type system",
      "Postconditions don't apply to subtypes, only preconditions do — the base type's output guarantees are owned by the base type and cannot be inherited or overridden",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Add a runtime check inside Square that throws if width and height ever differ, so callers know to catch the error when they try to set dimensions independently",
      "Make Rectangle's width and height properties private so Square can't override them, forcing it to inherit the storage and abandoning the equal-dimension invariant entirely",
      "There is no fix; Square and Rectangle must never coexist in the same codebase because geometric is-a relationships can never be made behaviorally consistent through any language mechanism",
    ],
    answer: 0,
    explanation:
...
class BrokenCache: Cache {
    ...
    options: [
      ...
      "It fails to compile because randomElement() isn't allowed in protocol conformances — associated-type constraints prevent non-deterministic operations from being used in conforming methods",
      "It's not actually an LSP issue since Cache is a protocol, not a class — LSP applies only to inheritance hierarchies where a subtype literally substitutes for a supertype",
      "Swift automatically detects postcondition violations for Dictionary-backed types by comparing the stored key at write time against the key supplied at read time using runtime assertions",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
