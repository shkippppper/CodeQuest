...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "Makes the parameter optional, so you can omit it entirely and Swift substitutes nil for the message inside the function body",
      "Marks the parameter as unused so the compiler suppresses the unused-variable warning without requiring a discard assignment",
      "Gives the parameter a default value of an empty string, so calling `log()` with no arguments is valid and prints a blank line",
    ],
    answer: 0,
    explanation:
...
connect(retries: 5)`,
  ...
  },
  {
    ...
    options: [
      ...
      "Because `self` is otherwise undefined inside closures and the explicit qualifier is needed to resolve the name through the capture environment",
      "It's a purely stylistic rule with no semantic meaning — the compiler accepts implicit self inside escaping closures but the style guide recommends the explicit form",
      "To make the closure run on the main thread, since Swift uses the presence of `self.` as a signal to dispatch the closure body through DispatchQueue.main automatically",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
