...
copy.append(4)        // (C) mutate`,
  ...
  },
  {
    ...
    options: [
      ...
      "Every distinct value must hash to a unique value — collisions are treated as correctness failures",
      "`hash(into:)` must combine every stored property, even ones excluded from the `==` implementation",
      "The type must be a class, because value types can't guarantee stable hash values across mutations",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "It runs the closure across multiple threads, using a concurrent dispatch queue to parallelize grouping work",
      "It is lazy and only computes on demand, deferring work until the resulting collection is first accessed",
      "It guarantees a stable insertion ordering that the regular `reduce` variant deliberately does not preserve",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
