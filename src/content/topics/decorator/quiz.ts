...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "A subclass that overrides every method of its parent class to intercept calls and inject additional behavior before delegating up",
      "A global singleton that all objects in the app reference in order to access a shared pool of augmented behaviors",
      "A protocol with no default implementations that forces every conforming type to independently re-implement all shared behavior",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Swift removed subclassing support for protocol-conforming types starting in Swift 5.5, making hierarchies invalid",
      "Subclasses technically can't override methods that are defined in protocol extensions, which all toppings use",
      "Structs are the only supported value type and can't be subclassed, so the pattern is architecturally impossible in Swift",
    ],
    answer: 0,
    explanation:
...
_ = try await loader.load(url: url)   // second call: hit`,
    options: [
      ...
      "No — the logger is wired directly to the URLSession layer, so it only fires when a real network request is dispatched to the server",
      "It throws an error on the second call because the cache and logger both try to own the same underlying URLSession task simultaneously",
      "Only the first call logs because the logger registers the URL as seen and skips all subsequent calls to that same URL",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Run measurably faster in all cases because decorator dispatch avoids the vtable lookup overhead of subclass method calls",
      "Avoid using protocols entirely, since decorators use direct type composition rather than abstract interface constraints",
      "Guarantee thread safety automatically, because each decorator wraps calls in a serial dispatch queue before forwarding them",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Swift automatically inserts a forwarding call to the wrapped object's method because the decorator conforms to the same protocol",
      "It compiles and always returns a stale cached value from a previous successful load stored by the surrounding cache layer",
      "This is a compile-time error because Swift requires decorators to call through to their wrapped object or the build fails",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
