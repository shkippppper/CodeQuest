...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "Stack is shared across threads, so all concurrent tasks can access the same local variables without copying; heap is per-thread to avoid contention",
      "Heap allocation is always faster because it uses a pre-mapped virtual memory region that avoids the pointer-bump overhead of stack frame management",
      "They are the same region — Swift uses a unified memory model where the compiler decides the physical location based on escape analysis at link time",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Always on the heap, because the Swift runtime boxes every value type to ensure its address remains stable across function calls and closures",
      "In a global table maintained by the Swift runtime that maps type identifiers to their most recent instantiated value for each thread",
      "In the CPU registers only, because the optimizer eliminates stack allocation entirely for small structs that do not escape their defining scope",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "The instance is on the stack; the variable holds a copy that is value-typed and does not participate in ARC, just like a struct would",
      "Both instance and variable are in registers, because the optimizer allocates small class instances entirely in register storage to skip ARC overhead",
      "Class instances aren't stored anywhere at runtime — only their methods and properties are materialized, and the variable holds a type descriptor, not an address",
    ],
    answer: 0,
    explanation:
...
let x: any Shape = HugeStruct()`,
    options: [
      ...
      "any is always the same speed as generics because the compiler specializes existential containers the same way it specializes generic functions when the conforming type is statically known",
      "HugeStruct can't be a Shape because protocol conformances for types with many stored fields require the type to explicitly opt into existential boxing by adding a conformance annotation",
      "It won't compile because assigning a value type larger than three words to an existential variable requires an explicit as! cast that the compiler rejects for non-final types",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
...
struct B { var b: Int64; var a: Bool; var c: Bool }`,
    options: [
      ...
      "Field order never affects size because the Swift compiler automatically reorders struct fields to minimize padding, making manual ordering irrelevant to the final layout",
      "Bool takes 8 bytes, because the Swift ABI guarantees that every stored property begins on an 8-byte boundary to match the natural word size of the underlying architecture",
      "Structs are always 24 bytes regardless of field count or order, because Swift aligns every struct to a minimum of three words to simplify the existential container layout",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "stride is smaller and saves memory, because it strips the trailing padding that size includes for struct types that contain reference-counted fields",
      "size is only valid for classes, because value types have no fixed runtime representation and their byte count is computed lazily by the MemoryLayout infrastructure",
      "stride includes the ARC header added in front of each heap-allocated element, making it larger than size by exactly the two-word refcount prefix",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
