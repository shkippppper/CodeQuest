...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "Turns it into a struct for value semantics",
      "Generates a UIKit view controller",
      "Makes every property @Published automatically",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "ModelContainer ≈ NSManagedObjectContext; ModelContext ≈ NSFetchRequest",
      "ModelContext is only used for reading, never for inserting objects",
      "ModelContainer is the SwiftUI view that displays the data",
    ],
    answer: 0,
    explanation:
...
context.insert(task)`,
    options: [
      ...
      "The task is already persisted to disk",
      "insert() throws unless save() is called in the same expression",
      "The task is discarded because no @Query is observing it",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "SwiftData requires manual inverse configuration; Core Data infers it automatically",
      "Neither framework supports two-way relationships",
      "Inverses are irrelevant in both frameworks unless using CloudKit",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Nothing — .lightweight stages handle any schema change automatically",
      "Manually deleting the ModelContainer's store on every app launch",
      "SwiftData has no migration support; you must export to JSON and reimport",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
...
// team wants to add @Model types reading/writing the same data`,
    options: [
      ...
      "No — SwiftData and Core Data use fundamentally incompatible storage, a full re-import is unavoidable",
      "Yes, but only if the app deletes all existing user data first",
      "No — SwiftData only works with brand-new, empty stores",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
