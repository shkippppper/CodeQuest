...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "It makes requests execute faster over the network by reusing a single URLSession configuration, avoiding the overhead of creating a new session on every individual API call",
      "It removes the need for a base URL by encoding the full absolute path directly inside each Endpoint value at definition time",
      "It automatically retries failed requests by wrapping each URLRequest in an observable that re-fires on transient network errors",
    ],
    answer: 0,
    explanation:
...
try decoder.decode(Article.self, from: json)`,
    options: [
      ...
      "It silently decodes publishedAt as nil, which compiles only if the property is declared as an Optional Date in the struct",
      "It automatically converts snake_case to camelCase because URLSession applies a key normalization pass before invoking JSONDecoder",
      "It ignores the mismatched key entirely and substitutes today's Date as a fallback, since Date has a sensible default value",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Because Swift requires all thrown errors within a module to conform to the same type for the compiler to perform exhaustiveness checks on every catch site",
      "It makes the network requests faster by reducing the number of try/catch branches the runtime evaluates per response",
      "It removes the need for a status-code check, since mapping to an enum folds the HTTP code into the thrown error automatically",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "401 is not a valid HTTP status code — it was deprecated in favor of 403 Forbidden, which is now the correct and widely accepted code for all authentication failures",
      "It makes JSON decoding faster by signaling to the decoder that no payload will accompany the error response body",
      "There's no real difference — badStatus(401) would behave identically everywhere because callers pattern-match on the associated Int",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "It guarantees the request will eventually succeed because the server is always reachable given enough elapsed time, enough retry attempts, and a stable network connection",
      "It fully satisfies the HTTP spec requirement that all compliant clients must implement exponential backoff before retrying any 5xx or connection timeout response",
      "It makes JSONDecoder parse responses faster on subsequent retry attempts by allowing the device CPU thermal state to recover fully between individual retry iterations",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
