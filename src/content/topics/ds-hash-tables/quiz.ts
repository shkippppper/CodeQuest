...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "It secretly keeps all keys sorted in a balanced BST and uses binary search to find any key in O(log n) comparisons",
      "It caches the last 10 lookups in a small linear buffer and returns the cached result for repeated queries without recomputing the hash",
      "Swift compiles Dictionary subscript lookups directly into a single machine-code instruction with no runtime hash computation or branch overhead",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Chaining is only used internally by Swift's Dictionary type, while open addressing is reserved exclusively for Swift's Set type and cannot apply to any other hash-based collection",
      "Open addressing allocates a linked list of overflow buckets outside the main array for each collision, while chaining stores all values inline in a flat contiguous array at a fixed computed offset from the bucket index",
      "Separate chaining and open addressing are simply two interchangeable names for the same underlying collision resolution algorithm, and the terms can be used synonymously in any technical discussion of hash table internals",
    ],
    answer: 0,
    explanation:
...
print(s.count)`,
    options: [
      ...
      "It always crashes at runtime with a fatal error because Swift's Set eagerly validates the Hashable contract on every insert and detects the y-ignoring violation immediately",
      "The second insert silently discards the new element and the Set retains only one entry, because Swift treats two values with identical hash codes as definitionally equal keys regardless of the == result",
      "It always prints 1 regardless of the y field values, because the Set deduplicates both genuinely different values solely on the basis of their identical hash codes without consulting the == operator",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Resizing a hash table is actually O(1) because Swift only swaps a single pointer to the new backing array and copies elements lazily on the next access rather than rehashing them all upfront",
      "Swift performs all hash table resize and rehash operations on a background GCD thread automatically, so the O(n) rehash cost never counts toward the time complexity of the caller's insert operation",
      "Resizing does make every insert O(n) in the worst case, and there is no valid amortization argument that can reduce the per-insert time complexity when the resize frequency is considered",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Sort the array first with a comparison sort — this alone reduces the problem to O(n) because the sorted order lets a single left-to-right scan identify all valid pairs without any additional data structure",
      "Modern CPUs expose SIMD vector instructions that allow a Swift Dictionary to search all n potential pairs simultaneously in a single parallel hardware operation, achieving true O(1) lookup across the whole array",
      "Two-sum cannot be solved better than O(n squared) in the general case without extra space, because every candidate pair must be explicitly considered at least once to guarantee a correct and complete answer",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
