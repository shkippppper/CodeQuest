...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "An object referenced from too many places, causing its retain count to exceed the system's supported maximum",
      "A loop in code that runs too long and starves the main thread, triggering a watchdog timeout",
      "A stack overflow caused by a recursive function calling itself without a valid base case",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "The app crashes immediately with a bad access exception on the leaked object",
      "A compile error flagging the strong reference cycle before the binary is produced",
      "Slow launch time, because ARC must walk the full reference graph on every startup",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "reload() is missing a body, and Swift requires all methods declared in a class to have a non-empty implementation",
      "Closures can't capture self unless the class is marked @escaping at the declaration site",
      "It doesn't leak — ARC detects when the only remaining references are part of a cycle and breaks them automatically",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "To make delegate method dispatch faster by eliminating the ARC retain and release overhead incurred on every individual method call",
      "Because delegates are always value types and weak is the only valid ownership qualifier applicable to a stored struct property",
      "Because delegate protocols must be AnyObject-constrained, and all AnyObject-constrained optional properties must be declared weak in Swift",
    ],
    answer: 0,
    explanation:
...
run { self.doThing() }`,
    options: [
      ...
      "Yes — any strong capture of self inside any closure creates a retain cycle, regardless of whether the closure escapes",
      "Only on classes; capturing self in a non-escaping closure still leaks when the enclosing type is a reference type",
      "It won't compile — the compiler requires [weak self] on every closure that references self, escaping or not",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "The parent's `child` reference — making it weak lets the child be freed independently while the parent is still alive",
      "Both sides, to be safe — making every inter-object reference weak ensures no leaks can occur",
      "Neither — use strong both ways and rely on the parent's deinit to nil out the child reference manually",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "The Swift compiler, which performs static analysis at build time and reports retain cycles as errors",
      "SwiftLint, which has built-in rules that flag strong reference patterns likely to form cycles",
      "The App Store review process, which runs a memory leak scan and rejects binaries with detected cycles",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
