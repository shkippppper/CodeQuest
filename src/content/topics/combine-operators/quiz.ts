...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "A special subscriber that attaches to a pipeline and silently discards every incoming value without firing any callback",
      "A global free function that mutates an existing publisher object in place by appending a new transformation step to its buffer",
      "A required protocol that every Subscriber conforming type must implement in order to register itself with any publisher for value delivery",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "`map` always runs asynchronously on a background dispatch queue while `flatMap` runs synchronously on the calling thread",
      "`flatMap` can only be used with array-backed publishers such as sequence publishers, never with network or subject-based sources",
      "They are completely interchangeable in every real-world case because the Swift compiler automatically unwraps any nested publisher produced by map",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Combine buffers user 2's result in an internal queue until user 1's response arrives, preserving arrival order",
      "The pipeline cancels user 2's publisher and throws a runtime ordering error downstream to the subscriber",
      "Only the first inner publisher ever emits, because flatMap keeps at most one concurrent subscription by default",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
...
letters.combineLatest(numbers).sink { print("latest: \\($0)\\($1)") }`,
    options: [
      ...
      "zip and combineLatest both print exactly A1, B2 — they differ only in internal buffering strategy but always produce the same output",
      "zip: A1, B1, B2 because it buffers unmatched values and re-emits. latest: A1, B2 because it waits for fully matched positions only",
      "Neither operator prints anything at all until both streams have completed their entire sequence and each sent a finished completion event",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "It automatically retries the upstream publisher a configurable number of times via its retry parameter before emitting the fallback",
      "It only converts the upstream Failure type to Never without actually recovering from or replacing the failed stream at all",
      "It suspends the entire pipeline in a waiting state until the subscriber manually clears the error condition and signals readiness",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "The next keystroke triggers a new search normally, because catch only terminates the specific failed inner publisher and keeps the outer stream alive",
      "The pipeline calls fatalError on the very next keystroke because a terminated upstream publisher can never be resumed or resubscribed",
      "catch only ever intercepts the very first failure event and then silently passes all subsequent errors straight through to the sink unchanged",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
