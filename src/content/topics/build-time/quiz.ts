...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "Immediately split the app into more modules",
      "Enable Whole Module Optimization in Debug",
      "Delete derived data and do a clean build every time",
    ],
    answer: 0,
    explanation:
...
let c = b + 2 + 3 + 4 + 5 + 6 + 7 + 8 + 9 + 10`,
    options: [
      ...
      "`c` — the extra `let b` declaration adds overhead",
      "They take identical time",
      "Neither compiles",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Modules always compile in parallel automatically with zero configuration",
      "Modules disable type inference, making compilation trivial",
      "It has no build-time effect, only organizational benefit",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "WMO is not supported outside of Release builds at all",
      "It causes compiler crashes on every build",
      "It has zero effect either way, so there is no downside",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
...
let user = fetchUser(id: "42")`,
    options: [
      ...
      "Changing a comment or local variable name inside fetchUser's body",
      "Both changes always trigger identical rebuild scope",
      "Neither change ever affects FileB.swift",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
