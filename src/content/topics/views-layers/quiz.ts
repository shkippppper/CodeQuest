...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "The view draws all visual content directly; the layer merely handles touch forwarding to UIKit",
      "They are fundamentally the same object — UIView is just a thin alias for CALayer on iOS",
      "The layer handles Auto Layout constraints while the view handles the actual rendering pipeline",
    ],
    answer: 0,
    explanation:
...
layer.shadowOpacity = 0.3`,
    options: [
      ...
      "Shadows require a UIView host and cannot be rendered on a standalone CALayer without a full UIView hierarchy attached to it",
      "shadowOpacity must be set to exactly 1.0 at all times; any fractional value between 0 and 1 is silently discarded by the Core Animation compositor on device",
      "It's a simulator-only Metal rendering bug that disappears entirely on a real device because the GPU pipeline handles clipping differently than the software rasterizer",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Always, on every layer, including UIView-backed layers, as long as the change happens on the main thread",
      "Never — you must always explicitly add a CABasicAnimation or CAKeyframeAnimation to trigger any animation",
      "Only inside a UIView.animate block, where Core Animation intercepts property changes for both views and layers",
    ],
    answer: 0,
    explanation:
...
layer.add(a, forKey: nil)   // model layer.position never set`,
    options: [
      ...
      "The animation duration of exactly 1 second is too short for Core Animation to complete a position transition; the minimum supported duration for spatial animations is 2 seconds",
      "CALayer cannot animate the position property at all; only the transform and opacity animatable properties support both implicit and explicit animation via CABasicAnimation",
      "You must call layoutIfNeeded on the superlayer after adding the animation to commit the position change from the presentation layer into the model layer's render tree",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "shadowPath overrides the shadowColor property and lets you encode a custom tint directly inside the CGPath's winding rule metadata",
      "shadowPath is required for the shadow to appear at all; without an explicit path, shadowOpacity is silently ignored by the Core Animation compositor on device builds",
      "shadowPath disables the existing shadow by replacing the rendering region with a zero-area CGPath, which is why setting it incorrectly causes nothing to render",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Always, for any layer — shouldRasterize reduces compositing cost in every scenario regardless of how frequently the content changes",
      "Only for layers that display text, since rasterization resolves subpixel antialiasing conflicts with the GPU renderer",
      "Never — shouldRasterize was removed in iOS 14 and setting it true has no effect on modern Apple Silicon",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
