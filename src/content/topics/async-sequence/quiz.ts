...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "A single value returned faster by skipping the suspension overhead that a plain async function incurs",
      "Synchronous iteration over a pre-computed collection without any asynchronous steps between elements",
      "Thread creation — each element of the sequence causes a new background thread to be spawned automatically",
    ],
    answer: 0,
    explanation:
...
for await n in stream { print(n) }`,
  ...
  },
  {
    ...
    options: [
      ...
      "To run units of work on a background thread without requiring the caller to await their completion",
      "To replace actors as the preferred synchronization primitive when state is read-only between events",
      "To cache network responses in memory so that repeated async calls return the stored value instantly",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: ["`AsyncThrowingStream`", "`AsyncStream` with a manual error flag", "`Result<T, Error>` wrapped in an AsyncStream", "`ThrowingAsyncSequence`"],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Nothing — AsyncStream applies true backpressure and blocks the producer automatically when the buffer is full",
      "Call `finish()` after each yield to reset the stream's buffer before the next value is enqueued",
      "Wrap the producer in a serial DispatchQueue so it naturally slows to the consumer's processing rate",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "To yield one final summary value before the stream ends, so consumers always receive a terminating element",
      "To upgrade a plain AsyncStream to a throwing one without reconstructing the continuation from scratch",
      "To move the stream's continuation onto the main actor so all yields are safely dispatched to the UI thread",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
...
task.cancel()`,
    options: [
      ...
      "The loop simply keeps running to completion, iterating until the underlying network byte stream naturally reaches its own end",
      "The app crashes",
