...
print(a.x, b.x)`,
  ...
  },
  {
    ...
    options: [
      "Because classes ignore `let` and treat all properties as mutable regardless of binding",
      "Because `let` freezes the reference, not the object it points to",
      "Because the property is implicitly `var` on all classes unless explicitly annotated `let`",
      "You can't — assigning to a property through a `let` class binding is a compile error",
    ],
    answer: 1,
    explanation:
...
print(a.tag, a.box.n)`,
  ...
  },
  {
    ...
    options: [
      ...
      "Structs cache all their method implementations in a per-type table that is populated at app launch",
      "Classes must re-read their vtable entries from disk on each call because they are not cached in memory",
      "There is no possible performance difference between struct and class method dispatch in optimized builds",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
