...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "WebSockets use TLS encryption built into the protocol upgrade handshake, whereas plain HTTP requests are always transmitted unencrypted",
      "Polling over HTTP is explicitly disallowed on modern mobile carrier networks due to bandwidth-conservation policies enforced at the radio layer",
      "WebSockets consume less battery than any HTTP request because the persistent connection bypasses the cellular radio's high-power state transitions",
    ],
    answer: 0,
    explanation:
...
socket.disconnect() // happens moments later, unrelated callback`,
    options: [
      ...
      "The message definitely was not delivered, because the connection dropped before the server could have written an acknowledgment",
      "The message was definitely delivered, because the local send callback fired without an error before the disconnect happened",
      "The socket layer automatically queues and retries the message after reconnecting, guaranteeing exactly-once delivery to the server",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "So the UI can sort messages alphabetically by their client-generated id and maintain a consistent visual order in the conversation",
      "So the server can count outgoing messages per client id and apply per-user rate limiting or usage-based billing accurately",
      "Because the WebSocket upgrade protocol specification requires a UUID in every application-level frame header for session tracing purposes",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "Because WebSocket frames use a binary encoding that can't carry UTF-8 text, so messages must be stored and re-encoded before display",
      "Because SwiftUI's data-binding system cannot directly observe a URLSessionWebSocketTask and requires an intermediate storage layer",
      "To reduce server load by batching outgoing message receipts and letting the database deduplicate acknowledgments before forwarding them",
    ],
    answer: 0,
    explanation:
...
outbox.enqueue(typingEvent.id)`,
    options: [
      ...
      "The local database schema uses a fixed BLOB column for payloads and cannot store the variable-length enum fields that presence events require",
      "The WebSocket protocol separates message frames from control frames at the transport layer, and presence events can only travel in control frames which the outbox can't write to",
      "Writing presence events into the outbox would violate the requirement that every outbox entry carry a stable client-generated id, which typing events inherently lack",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "The outbox acquires an exclusive database write lock that prevents any second flush goroutine from reading pending rows until the first flush fully commits",
      "The server enforces a per-client rate limit that automatically rejects any two requests arriving within one second of each other as a duplicate-detection heuristic",
      "Duplicate messages are expected in this design and are instead deduplicated at render time by the UI diffing layer before they are displayed to the user",
    ],
    answer: 0,
    difficulty: "senior",
    explanation:
