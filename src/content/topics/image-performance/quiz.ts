...
const quiz: Question[] = [
  {
    ...
    options: [
      ...
      "Its compressed file size on disk, scaled by a per-format overhead factor",
...
imageView.image = image                     // line B, first draw`,
    options: [
      ...
      "Immediately at line A, synchronously blocking the calling thread until complete",
  ...
  },
  {
    ...
    options: [
      ...
      "NSCache is faster at every lookup because it uses a lock-free hash structure internally",
      "Dictionary cannot store UIImage values without boxing them in AnyObject first",
  ...
  },
  {
    ...
    options: [
      ...
      "UIImage caches too aggressively, holding decoded bitmaps in memory even after the cell is recycled and the image is no longer visible",
  ...
  },
  {
    ...
    options: [
      ...
      "Cancelling is required by NSCache's API before any new entry can be inserted under the same key",
      "Uncancelled loads block the background queue permanently, starving all subsequent image requests of thread capacity",
